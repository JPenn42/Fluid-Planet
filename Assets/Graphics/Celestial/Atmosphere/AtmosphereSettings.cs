...
public class AtmosphereSettings : ScriptableObject
{
	...
	public void SetProperties(Material material, float bodyRadius, Vector3 bodyPos, Vector3 dirToSun, bool force)
	{
		/*
		...
		*/
		...
		material.SetVector("planetCentre", bodyPos);

		if (!settingsUpToDate || !Application.isPlaying || force)
		{
	...
	void PrecomputeOutScattering()
	{
		...
		{
			ComputeHelper.CreateRenderTexture(ref opticalDepthTexture, textureSize, textureSize);
