    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(v2f i) : SV_Target
            {
                ...
                float bgDepth = SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, i.uv);
                float sceneDepth = LinearEyeDepth(bgDepth) * length(viewVector);
                //return bgDepth;
                ...
                float4 bg = tex2D(_MainTex, float2(i.uv.x, 1-i.uv.y));
                float4 foamData = tex2D(FoamMap, float2(i.uv.x, i.uv.y));
                float foam = foamData.r;
                float foamDepth = foamData.b;

                // ---- Get test-environment colour (and early exit if view ray misses fluid) ----
                float3 viewDirWorld = WorldViewDir(i.uv);
                float3 world = foamData.rgb;
                //return bg;
                if (depthSmooth > sceneDepth) return bg;
