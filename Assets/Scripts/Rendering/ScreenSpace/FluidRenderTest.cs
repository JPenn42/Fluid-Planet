...
namespace Seb.Fluid.Rendering
{
	public class FluidRenderTest : MonoBehaviour
	{
		...
		void BuildCommands()
		{
			...
			cmd.Blit(BuiltinRenderTextureType.CameraTarget, finalRt, matComposite);
		...
		void Init()
		{
			...
			void InitTextures()
			{
				...
				ComputeHelper.CreateRenderTexture(ref finalRt, width, height, FilterMode.Bilinear, fmtRGBA, depthMode: DepthMode.None);
		...
		void UpdateSettings()
		{
			...
			matComposite.SetTexture("FoamMap", foamRt);
		...
		void OnDestroy()
		{
			...
			ComputeHelper.Release(depthRt, thicknessRt, normalRt, compRt, shadowRt, foamRt, finalRt);
		}
	}
}